...
\begin{abstract}
  ...
  maps) for use in scientific visualization.  I provide a diverging
  ...
  rainbow color map for serious scientific visualization applications.  I
  also present an algorithm that allows users to easily generate their own
...
\begin{CRcatlist}
  \CRcat{I.3.m}{Computing Methodologies}{Computer Graphics}{Miscellaneous};
  \CRcat{H.5.2}{Information Interfaces and Presentation}%
    {User Interfaces}{Screen design};
  \CRcat{H.1.2}{Models and Principles}{User/Machine Systems}{Human factors}
\end{CRcatlist}
...
\begin{figure}
  \centering
  \includegraphics[width=.75in]{images/Cool2WarmLabRadial}
  \qquad
  \includegraphics[width=.75in]{images/Cool2WarmRadial}
...
\begin{equation}
  \cvec{c}(x) = \langle M \cos \operatorname{s}(x),
  M \sin \operatorname{s}(x) \cos h,
  M \sin \operatorname{s}(x) \sin h \rangle
  \label{eqn:s_linear}
\end{equation}
...
\begin{equation}
  \begin{gathered}
    \left\lVert \left\langle M s_m \sin\operatorname{s}(x),
        M s_m \cos\operatorname{s}(x) \cos h,
        M s_m \cos\operatorname{s}(x) \sin(h) \right\rangle \right\rVert \\
...
first converted to \Msh.\footnote{I do not provide the implementation of
\proc{RGB2Msh} or \proc{Msh2RGB}.  However, the conversion between \Msh and
\Lab is a straightforward implementation of Equations \ref{eqn:LabToMsh}
and \ref{eqn:MshToLab}.  Conversions between \RGB and \Lab can be found in
many books on color or web resources like
\href{http://www.easyrgb.com/}{EasyRGB.com}.}  To enforce a diverging color
map, white is added between the two control points
...
\begin{figure}
  ...
  \begin{tabular}{c}
    \subfigure[Simply pick two endpoint colors to create a diverging color
      map in between them.]%
	      {%
		\includegraphics[width=.45\linewidth]{images/tfOriginal}~%
...
\Lab values using a D65 white point and a gamma correction of 2.4.
...
Figure~\ref{fig:InterpolateColor} has been add to the
...
color map interpolation has also been add to ParaView, a free, open-source
