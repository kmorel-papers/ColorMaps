...
\newboolean{colorplate}
\setboolean{colorplate}{false}
...
\ifthenelse{\boolean{colorplate}}
           {\usepackage{plates}}
           {\usepackage[figures]{plates}}
...
rainbow color map\ifthenelse{\boolean{expanded}}{, shown in
  \autoref{fig:RainbowColorMap}}, which cycles through all of the most
...
\ifthenelse{\boolean{expanded}}{
  ...
  rainbow colors in numerous different ways~\cite{Ware04}.
  ...
  introduce artifacts and obfuscate real data~\cite{Borland07}.  This is
  demonstrated in \autoref{fig:RainbowSpatialContrast}.
...
\ifthenelse{\boolean{expanded}}{
  ...
  shown in \autoref{fig:GrayscaleColorMap}.  Completely devoid of any
  ...
  changes in luminance~\cite{Mullen85,Ware04}.  The grayscale color map is
  ...
  20\%~\cite{Ware88}.  This effect, demonstrated in
  \autoref{fig:SimultaneousContrast}, is called simultaneous
  contrast~\cite{Stone05}. Adding a chromaticity shift helps, but does not
...
\ifthenelse{\boolean{expanded}}{
  ...
  \autoref{fig:IsoluminantColorMap}.  Somewhat opposite to the grayscale
  ...
  in \autoref{fig:LuminanceVsShading}.
  ...
  in luminance, especially for high frequency data~\cite{Rogowitz96}.
  ...
  frequency data~\cite{Rogowitz96}.

}

These color maps comprise those most commonly used in the literature and
tools today.  Other color maps are proposed by Ware~\cite{Ware04} as well
...
3-tuple~\cite{Stone03}.  This result is a side effect
...
more spaces in more detail\ifthenelse{\boolean{expanded}}{}{ with
  conversions between them}~\cite{Stone03,Wyszecki82}.
...
the blending and interpolation of colors~\cite{Stone03}.  When computing
...
\ifthenelse{\boolean{expanded}}{
  ...
  ambiguous~\cite{Fortner97}.  For the purposes of this paper, we will
  ...
  programs~\cite{Stone03}.  The conversion from the sRGB components to \RGB
  ...
  \begin{equation}
    \begin{gathered}
      \begin{aligned}
        ...
        b* &=
          200 \left[ \operatorname{f}(Y/Y_n) - \operatorname{f}(Z/Z_n) \right] \\
      \end{aligned} \\
...
\ifthenelse{\boolean{expanded}}{
  ...
  \begin{figure}
    ...
    \caption{Examples of color maps from Brewer~\cite{Brewer05}.}
  ...
  \autoref{fig:BrewerExamples}.  The qualitative color maps (also known as
  nominal color maps~\cite{Ware04}) are used to represent a collection of
  ...
  The sequential color maps (also known as ordinal~\cite{Ware04} or
  saturation~\cite{Rheingans99} color maps) are (nearly) monochromatic.  They
...
regard to perception.  The examples here adjust the hue to be away from 0
(except in the purple hues) because it provides slightly more aesthetically
pleasing results.
\ifthenelse{\boolean{expanded}}{
\autoref{fig:AdjustHue} gives a simple algorithm for
adjusting the hue.
...
\autoref{fig:hue_spin_plot} shows the effects of applying a hue spin.
The plot is smooth, perceptually uniform, and comparable to the plot given
in \autoref{fig:s_plot}.  In addition, the hue spin allows the entire
...
The \proc{InterpolateColor} algorithm in \autoref{fig:InterpolateColor}
...
using the \proc{AdjustHue} function in \autoref{fig:AdjustHue}.
...
build the cool to warm color map shown in \autoref{fig:Cool2WarmBar}.
The control points, to be interpolated in \Msh space, are given in
\autoref{table:Cool2Warm}.
\ifthenelse{\boolean{expanded}}{
  \autoref{table:Cool2WarmRGB} gives example interpolated \RGB values for
...
%% , as demonstrated in \autoref{fig:Cool2WarmResponse}.
