...
However, each has its sets of features and detractors, and the choice of
...
displayed, and it is sometimes useful to define colors over everything a
human can see.  To this end the International Commission on Illumination
(better known as CIE from its French title, the \emph{Commission
  Internationale de l'\'{E}clairage}) convened in 1931 and defined the \XYZ
color space \sticky{Reference}.  The \XYZ color space provides a convenient
way to consistently describe all visible colors without being tied to any
...
Our ultimate goal is to design a color map that works well in
...
The sequential color maps (also known as saturation color
maps\lcite{Rheingans99}) are (nearly) monochromatic.  They range from a
heavily saturated color to various levels of unsaturation.  Luminance is
also often increased as saturation is decreased so that the color map
terminates in a color at or close to white.  The monotonic nature of the
...
The diverging color maps (also known as bipolar\sticky{Reference
  from\lcite{Ware04}} or double-ended\lcite{Rheingans99}) have two major
...
ends of the visibile spectrum refract in the eye\lcite{Bailey06}, an effect
...
Using our initial criteria, we can think of diverging color maps as
...
vision.  The map also has a good perceptual range and minimally interfears
