...
colors will change faster near white when $M$ is larger.  Dropping the $M$
for the middle white value to be consistent will make the color changes
consistent again, but at the expense of the brightness of the color map.
In practice it is difficult to choose a single value for $M$ that yields
colors all within a typical display gamut and still provides pleasingly
bright colors.

\sticky{The usefulness of the spin is somewhat diminished by the fact that
  I am also lowering the luminance of white from that shown in
  Figure~\ref{fig:s_plot}.  However, I had to do that to make the effects
  of the gamut clipping noticeable.}
...
counterbalance the growing $M$ (although a large change can still cause a
noticeable pointing of the luminance).  The next section describes how to
choose an appropriate hue change.
